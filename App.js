...
function TelaPrincipal() {
  ...
  const buscarCotacoes = async () => {
    ...
    try {
      ...
      const listaMoedas = Object.keys(dados).map(key => ({
        codigo: key,
        nome: dados[key].name,
    ...
    } catch { Alert.alert('Erro', 'Falha ao buscar'); }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#eef2f5' }}>
      ...
      <View style={{ flex: 1, padding: 20 }}>
        <View style={styles.subHeader}>
          ...
          <Text style={styles.subHeaderText}>Ultima atualizacao: {atualizadoEm || 'Carregando...'}</Text>
        </View>
        {carregando ? <ActivityIndicator size="large" color="#4CAF50" /> : (
          <ScrollView>
            {cotacoes.map((m, i) => (
              <View key={i} style={styles.card}>
                <View style={styles.moedaInfo}>
                  <Image
                    ...
                    style={[
                      styles.bandeira,
                      m.codigo === 'DOGE' || m.codigo === 'BTC' || m.codigo === 'ETH' ? styles.bandeiraCrypto : null
                    ]}
                  />
                  <View>
        ...
        <TouchableOpacity style={styles.botao} onPress={buscarCotacoes} disabled={carregando}>
          <Text style={styles.botaoTexto}>{carregando ? 'Atualizando...' : 'Atualizar Cotações'}</Text>
        </TouchableOpacity>
      </View>
      <View style={styles.bottomBar}>
        ...
        <View style={styles.bottomBarItem}>
          <Text style={styles.bottomBarIcon}>📊</Text>
          <Text style={styles.bottomBarText}>Cotações</Text>
        </View>
        <View style={styles.bottomBarItem}>
          <Text style={styles.bottomBarIcon}>👤</Text>
...
const styles = StyleSheet.create({
  webContainer: { flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#e0e0e0' },
  appWrapper: { flex: 1, width: '100%', backgroundColor: '#fff'},
  container: { flex: 1, padding: 25, justifyContent: 'center', backgroundColor: '#f5f7fa' },
  titulo: { fontSize: 28, fontWeight: 'bold', color: '#2c3e50', textAlign: 'center', marginBottom: 30 },
  input: { borderWidth: 1.5, padding: 12, marginBottom: 20, borderRadius: 12, borderColor: '#dcdde1', backgroundColor: '#fff', fontSize: 16 },
  button: { backgroundColor: '#3498db', padding: 15, marginTop: 10, borderRadius: 12 },
  buttonText: { color: '#fff', textAlign: 'center', fontWeight: 'bold', fontSize: 16 },
  linkText: { color: '#3498db', textAlign: 'center', fontSize: 14, fontWeight: '600', marginTop: 20 },
  errorText: { color: '#e74c3c', fontSize: 13, marginBottom: 10, textAlign: 'center' },
  header: { backgroundColor: '#2f3c7e', padding: 20, alignItems: 'center' },
  headerTitle: { color: '#fff', fontSize: 20, fontWeight: 'bold' },
  subHeader: { backgroundColor: '#fff', padding: 15, borderRadius: 10, marginBottom: 20, alignItems: 'center' },
  subHeaderTitle: { fontSize: 16, fontWeight: 'bold' },
  subHeaderText: { fontSize: 12, color: '#666' },
  card: { backgroundColor: '#fff', padding: 20, borderRadius: 15, marginBottom: 15, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  moedaInfo: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  bandeira: { width: 32, height: 24, resizeMode: 'contain' },
  moeda: { fontSize: 16, fontWeight: 'bold' },
  moedaCodigo: { fontSize: 12, color: '#666' },
  valor: { fontSize: 20, color: '#2f3c7e', fontWeight: 'bold' },
  botao: { backgroundColor: '#4CAF50', padding: 15, borderRadius: 30, marginTop: 20, alignItems: 'center' },
  botaoTexto: { color: '#fff', fontWeight: 'bold', fontSize: 16 },
  bottomBar: { flexDirection: 'row', backgroundColor: '#fff', paddingVertical: 12, borderTopWidth: 1, borderTopColor: '#e0e0e0', justifyContent: 'space-around', alignItems: 'center' },
  bottomBarItem: { alignItems: 'center', gap: 4 },
  bottomBarIcon: { fontSize: 24 },
  bottomBarText: { fontSize: 12, color: '#666' },
});
